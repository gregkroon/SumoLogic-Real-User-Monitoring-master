...
(function(w) {
	...
	boomr = {
		...
		real_sendBeacon: function() {
			...
			//****************************************************************************
			//Start Sumo code change

			var ttfb = impl.vars["nt_res_st"] - impl.vars["nt_nav_st"];
			var pageload = impl.vars["nt_load_st"] - impl.vars["nt_nav_st"];
			var latency = impl.vars["nt_res_st"] - impl.vars["nt_fet_st"];
			var domainlookup = impl.vars["nt_dns_end"] - impl.vars["nt_dns_st"];
			var serverconnecttime = impl.vars["nt_con_end"] - impl.vars["nt_con_st"];
			var serverresponsetime = impl.vars["nt_res_st"] - impl.vars["nt_req_st"];
			var pagedownloadtime = impl.vars["nt_res_end"] - impl.vars["nt_res_st"];
			var dominteractive = impl.vars["nt_domint"] - impl.vars["nt_nav_st"];
			var domcontentloadtime = impl.vars["nt_domcontloaded_st"] - impl.vars["nt_domloading"];
			var domprocessingtointeractive = impl.vars["nt_domint"] - impl.vars["nt_domloading"];
			var dominteractivetocomplete = impl.vars["nt_domcomp"] - impl.vars["nt_domint"]; 
			var onload = impl.vars["nt_load_end"] - impl.vars["nt_load_st"]; 
			var domcomplete = impl.vars["nt_domcomp"] - impl.vars["nt_domloading"]; 
      			
            
            
            
      		BOOMR.addVar("sumo_timetofirstbyte", ttfb);
      		BOOMR.addVar("sumo_domainlookup-dns", domainlookup);
      		BOOMR.addVar("sumo_serverconnecttime-tcp", serverconnecttime);
      		BOOMR.addVar("sumo_domcontentloadtime", domcontentloadtime);
      		BOOMR.addVar("sumo_domprocessingtointeractive", domprocessingtointeractive);
      		BOOMR.addVar("sumo_domcomplete", domcomplete);
			BOOMR.debug("Ready to send beacon: " + BOOMR.utils.objectToString(impl.vars));

			//End Sumo Code Change

			//if (!impl.beacon_url) { changed to if (impl.beacon_url) { to avoid native boomerang beacon being sent .
			if (!impl.beacon_url || !impl.beacon_url) {
