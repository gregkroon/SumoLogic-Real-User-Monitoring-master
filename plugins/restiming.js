...
(function() {
  ...
  function restimingDecoder(restiming) {
    ...
    //var arr = {resourcename: , resourcetiming: []};
    var arr = {};
    var i = 0;
    ...
    if (number_of_worst_entries === -1) {
      assets.worst_entries = assets.entries.slice();
    } else {
      assets.worst_entries = assets.entries.slice(0, number_of_worst_entries);
    }
    assets.entries = undefined;

    // display it as human-readable
    assets.worst_entries = assets.worst_entries.map(function (item) {
    //	temp = assets.worst_entries.map(function (item) {


    
//Start of Sumo Code Change
     t = "\\\"" + "resourcetime" + i + "\\\"" + ":" + "{" + "\\\"" + item.name + "\\\"" + ":" + "\\\"" + item.time + "\\\"" + "}";
//End of Sumo Code Change
	...
	function addToBeacon(r) {
		...
		var SumoBeacon = {};
    	SumoBeacon.rum = {};
    	SumoBeacon.rum.t_done = r.t_done;
    	SumoBeacon.rum.t_resp = r.t_resp || undefined;
    	SumoBeacon.rum.t_page = r.t_page || undefined;
    

		if (r.restiming) {
		SumoBeacon.rum.restiming = restimingDecoder(r.restiming);
		}

		if (r.t_other !== undefined) {
     	 var others = r.t_other.split(",");
     	 var t_other = {};
     		 for (var i = 0; i < others.length; i++) {
       			 var item = others[i].split("|");
        		 t_other[item[0]] = parseInt(item[1]);
      	}
      	SumoBeacon.rum.RT = t_other;
    	}
    	

    	var replaceString = JSON.stringify(SumoBeacon.rum.restiming.worst_entries);
		replaceEscape = replaceString.replace(/\\/g,'');
		

		BOOMR.addVar("restiming", "{" + SumoBeacon.rum.restiming.worst_entries + "}");
	}	
		//************************************************************************************
